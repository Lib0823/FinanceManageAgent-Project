...
| `api-server/` | 백엔드 API / 매매 실행 | Spring Boot 4.1, JPA, Spring Security | 8080 |
...
데이터 흐름과 일일 파이프라인 등 상세 아키텍처는 `CLAUDE.md`를 참고하세요.
...
FinanceManage_Agent/
├── web-app/        # Vue 3 프론트엔드
├── api-server/     # Spring Boot 백엔드 (Gradle Wrapper 포함)
├── ai-agent/       # FastAPI ML 파이프라인
├── database/       # PostgreSQL 스키마 (schema.sql)
├── docker-compose.yml
├── CLAUDE.md       # 아키텍처 / 데이터 흐름 상세 문서
└── README.md
```
