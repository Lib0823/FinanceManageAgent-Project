...
| 기능 | 엔드포인트 | 상태 | 비고 |
|------|-----------|------|------|
| 종목 검색 | `GET /stocks/search?q=` | 완료 | code prefix OR name contains(ignore-case), 최대 30건 → `List<StockSearchResponse>` |
| 종목 현재가 | `GET /stocks/{stockCode}/price` | 완료 | `StockPriceResponse{currentPrice, changeAmount, changeRate, notice?}` |
| 실시간 호가 | `GET /stocks/{stockCode}/orderbook` | 완료 | KIS `FHKST01010200` → `OrderbookResponse{currentPrice, asks[10], bids[10], notice}` (10단계 매도/매수 + 잔량) |
