...
|---|---------|-------|------|----------|--------------|
| 1 | **stock_filter_score** | 1, 2-1 | 종목 필터링 점수 + 정량 피처 | 매일 | 30개 |
| 2 | **market_daily_summary** | 1 | KOSPI 지수 및 시장 요약 | 매일 | 1개 |
| 3 | **news_analysis** | 2-2 | 뉴스 감성 분석 결과 | 매일 | 30개 |
| 4 | **prophet_forecast** | 2-3 | Prophet 시계열 예측 | 매일 | 30개 |
| 5 | **ai_trade_decision** | 4 | Gemini AI 매매 결정 | 매일 | 6개 (TOP3×2) |
| 6 | **safety_filter_result** | 5 | 안전 필터 검증 결과 | 매일 | 6개 |
...
**용도**: KOSPI 100 → Top 30 필터링 점수 + 정량적 피처 저장
...
**Gemini 입력 피처**: `morning_return`, `close_position`, `foreign_net_buy`, `institutional_net_buy`

**저장 주기**: 매일 30개 종목 (Stage 1 INSERT → Stage 2-1 UPDATE)
...
### 6. safety_filter_result (Stage 5)
...
| `save_quantitative_features()` | stock_filter_score | 2-1 | 정량 피처 UPDATE |
