...
| **[USER_GUIDE.md](USER_GUIDE.md)** | 설치·환경변수·서버 실행·API 엔드포인트·모니터링·트러블슈팅 | 직접 실행/운영하는 사람 |
...
- **실행하고 싶다** → [USER_GUIDE.md](USER_GUIDE.md) §2 설치 → §3 실행 → §4 API
...
- **트리거**: 자동 스케줄(평일 08:50)은 **Stage 1만** 실행. 전체 파이프라인(Stage 1~6)은 `POST /api/pipeline/trigger`로 실행. ([STATUS.md](STATUS.md) §2-1)
